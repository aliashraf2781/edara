...
/**
 * Imports are per-cell, not all-or-nothing. Headline counts students; the
 * imported tile is individual subject results.
 */
    ...
    <div className="flex flex-col gap-6">
      <Card>
        <CardHeader
          ...
          action={
            onStartOver ? (
              <Button onClick={onStartOver}>{text.startOver}</Button>
            ) : undefined
          }
        />
        <CardBody className="flex flex-col gap-4">
          <p className="text-body text-ink">
            {clean
              ? text.allImported(report.valid_rows, report.total_rows)
              : text.summary(report.valid_rows, report.total_rows, report.invalid_rows)}
          </p>
